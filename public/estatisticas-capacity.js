...
const updateCapacityCard = (targetGroup) => {
    ...
    const groupsToShow = targetGroup === "Todos" ? Object.keys(capacities) : (capacities[targetGroup] ? [targetGroup] : []);
    groupsToShow.forEach(gName => {
        ...
        if (groupData && groupData.votes) {
            Object.values(groupData.votes).forEach(vData => {
                ...
                if (["Irei, ida e volta.", "Irei, mas não retornarei.", "Não irei, apenas retornarei."].includes(opt)) confirmations++;
            });
        }
        ...
        renderCompactBar(gName, confirmations, capacities[gName], Math.max(0, totalPassengers - votedCount));
        hasAnyCapacity = true;
    });
...
const renderCompactBar = (name, count, cap, pending) => {
    ...
    const busColors = [
        { grad: 'linear-gradient(90deg, #2196f3, #4caf50)' }, { grad: 'linear-gradient(90deg, #9c27b0, #00bcd4)' },
        { grad: 'linear-gradient(90deg, #fb8c00, #ffeb3b)' }, { grad: 'linear-gradient(90deg, #f44336, #e91e63)' },
        { grad: 'linear-gradient(90deg, #3f51b5, #2196f3)' }
    ];
    const barGrad = isFull ? 'linear-gradient(90deg, #f44336, #ff5252)' : busColors[Object.keys(capacities).indexOf(name) % busColors.length].grad;
    ...
    if (isFull) { infoDiv.style.color = "#ff5252"; infoDiv.style.fontWeight = "700"; }
    ...
    progressBar.style.cssText = "height: 100%; background: " + barGrad + "; border-radius: 4px; transition: width 1s cubic-bezier(0.4, 0, 0.2, 1); position: relative;";
