...
async function migrate() {
  if (!executeMigration || oldGroupName === "Ônibus unesc 01 2026") {
    console.log(
      '⚠️  Migração não executada. Edite o arquivo, defina os nomes e mude "executeMigration" para true.',
    );
    process.exit(0);
  }
  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      if (config.groupCapacities && config.groupCapacities[oldGroupName]) {
        config.groupCapacities[newGroupName] =
          config.groupCapacities[oldGroupName];
      ...
      console.log(
        "⚠️  Aviso: config/config.json não encontrado. Pulando etapa local.",
      );
    }
